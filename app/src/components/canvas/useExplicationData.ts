 ...
 *
 * ЕДИНСТВЕННЫЙ ИСТОЧНИК НОМЕРОВ: groupMap из этого хука передаётся в ObjectNumbers,
 * который НЕ выполняет собственную группировку. ObjectNumbers берёт готовые номера
 * через groupKey(obj) → groupMap.get(key) → number.
 *
 * Правила:
