 ...
 *
 * Кнопка «🎤 Голосом»: слушает фразу дачника («полил помидоры»), угадывает
 * тип события по ключевому глаголу и отдаёт остаток фразы как заметку —
 * `{ eventType: 'watering', note: 'помидоры' }`. Если Web Speech API не
...
const KEYWORDS: { type: string; words: string[] }[] = [
  ...
  { type: "harvest", words: ["собрал", "собрала", "урожай", "снял", "выкопал"] },
  { type: "fertilizing", words: ["подкорм", "удобр", "внёс", "внес"] },
  { type: "pruning", words: ["обрезал", "обрезала", "обрезка", "подрезал"] },
  { type: "transplant", words: ["пересадил", "пересадила", "пересадка"] },
  { type: "planting", words: ["посадил", "посадила", "посеял", "посеяла", "посадка"] },
  { type: "blooming", words: ["цвет", "зацвел", "расцвел"] },
  { type: "fruiting", words: ["плодонос", "завязал", "плоды"] },
